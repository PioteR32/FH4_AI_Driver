...
class SimpleDataset(Dataset):
    def __init__(self, main_path=None, main_name_of_folder=None):
        ...
        for folder in self.data_folders:
            ...
            if os.path.exists(csv_path):
                ...
                speeds = (df['Speed_kmh'].values.astype(np.float32) / 360.0).astype(np.float32)
                steering = df['Steering'].values.astype(np.float32)
                lt = df['LT'].values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        speeds = torch.from_numpy(data['speeds'][image_idx:image_idx + 5]) # float32
        label = torch.from_numpy(data['targets'][image_idx + 4]) # float32
