...
class OnlyTanhModel(nn.Module):
    ...
    def __init__(self, input_channels=3,num_of_img=5):
        super(OnlyTanhModel, self).__init__()
        ...
        self.bn5= nn.GroupNorm(num_groups=8, num_channels=512)
        
        # Pooling layer to reduce spatial dimensions
        self.pool = nn.AdaptiveAvgPool3d((1, 4, 4))
        
        # Fully connected layers for final prediction
        self.fc1 = nn.Linear(2*4096, 1024)
        self.fc2 = nn.Linear(1024, 512)
        self.fc3 = nn.Linear(512, 128)
        
        # Output layer - 3 outputs for steering, throttle, brake
        self.output_layer = nn.Linear(128 + num_of_img, 2)  # 5 for the speeds input
    ...
    def forward(self, x,speeds):
        ...
        throttle_brake = torch.tanh(output[:, 1])
        
        # Combine outputs
        combined_output = torch.stack([steering, throttle_brake], dim=1)
